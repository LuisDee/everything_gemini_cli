...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for your coding agent. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns agent subprocesses to fix violations the agent didn't catch.

## When to Use

- You want automatic formatting and linting on every file edit (not just at commit time)
- You need defense against agents modifying linter configs to pass instead of fixing code
- You want tiered model routing for fixes (Gemini 2.5 Flash for simple style, Gemini 2.5 Flash for logic, Gemini 2.5 Pro for types)
...
Every time the agent edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
|-- Spawns gemini --prompt subprocess with violations JSON
|-- Routes to model tier based on violation complexity:
|   |-- Gemini 2.5 Flash: formatting, imports, style (E/W/F codes) -- 120s timeout
|   |-- Gemini 2.5 Flash: complexity, refactoring (C901, PLR codes) -- 300s timeout
|   +-- Gemini 2.5 Pro: type system, deep reasoning (unresolved-attribute) -- 600s timeout
...
A PreToolUse hook on shell commands blocks legacy package managers:
...
```json
{
  ...
  "subprocess": {
    "tiers": {
      "flash":  { "timeout": 120, "max_turns": 10 },
      "flash_complex": { "timeout": 300, "max_turns": 10 },
      "pro":   { "timeout": 600, "max_turns": 15 }
    },
    "volume_threshold": 5
  }
}
