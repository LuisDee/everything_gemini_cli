...
- BeforeModel hook evaluates prompt
...
Create a dynamic research plan before asking questions.

**Research Plan Template:**
1. **Check conversation history first** - Avoid redundant exploration if context already exists
2. **Review codebase** if needed:
   - Explore for architecture and project structure
   - Use `grep_search`/`glob` for specific patterns, related files
...
3. **Gather additional context** as needed:
   ...
   - Search the web for best practices, common approaches, current information
...
Present your research-grounded questions to the user directly in conversational form.

**Question Format:**
Present questions with numbered options that the user can choose from:

```
I have a few questions to clarify your request:

**1. Which file should be modified?** (Target file)
   a) src/auth/login.ts - Main login handler (245 lines)
   b) src/auth/middleware.ts - Auth middleware for protected routes (89 lines)
   c) src/auth/session.ts - Session management utilities (156 lines)
   d) Other (please specify)

**2. What approach should we use?** (Approach)
...
**Important:** Always include an "Other" option. Keep questions focused on one decision point each.
