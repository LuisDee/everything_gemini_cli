...
#
# Detection priority:
#   1. GEMINI_PROJECT_DIR env var (if set), falls back to CLAUDE_PROJECT_DIR
...
_clv2_detect_project() {
  ...
  # 1. Try GEMINI_PROJECT_DIR env var (with CLAUDE_PROJECT_DIR fallback)
  local _project_dir_env="${GEMINI_PROJECT_DIR:-$CLAUDE_PROJECT_DIR}"
  if [ -n "$_project_dir_env" ] && [ -d "$_project_dir_env" ]; then
    project_root="$_project_dir_env"
    source_hint="env"
  fi
