...
| Feature | v1 | v2 |
|---------|----|----|
| Observation | AfterAgent hook (session end) | BeforeTool/AfterTool (100% reliable) |
| Analysis | Main context | Background agent (gemini-2.5-flash) |
...
+---------------------------------------------+
      |
      | Observer agent reads (background, gemini-2.5-flash)
      v
+---------------------------------------------+
|          PATTERN DETECTION                   |
...
1. **`GEMINI_PROJECT_DIR` env var** (highest priority, falls back to `CLAUDE_PROJECT_DIR`)
...
```json
{
  "hooks": {
    "BeforeTool": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "~/.gemini/skills/continuous-learning/hooks/observe.sh"
      }]
    }],
    "AfterTool": [{
      "matcher": "*",
...
- AfterAgent hook still runs (but now also feeds into v2)
