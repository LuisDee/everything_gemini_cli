...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${ECC_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping analysis on Windows due to known non-interactive hang issue (#295). Set ECC_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
...
PROMPT
  ...
  gemini --prompt "$(cat "$prompt_file")" >> "$LOG_FILE" 2>&1 &
  gemini_pid=$!
