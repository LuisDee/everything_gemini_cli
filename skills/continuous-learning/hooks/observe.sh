...
#
# Registered via hooks configuration (auto-loaded when enabled).
...
# Pass HOOK_PHASE via env var since the CLI does not include hook type in stdin JSON
...
try:
    ...
    # The CLI does NOT include a "hook_type" field in the stdin JSON,
