#!/usr/bin/env python3
"""Generate HTML report from description optimization loop output.

Shows each description attempt with check/x for each test case.
...
""" + refresh_tag + """    <title>""" + title_prefix + """Description Optimization</title>
    <style>
        body { font-family: -apple-system, system-ui, sans-serif; max-width: 100%; margin: 0 auto; padding: 20px; background: #faf9f5; color: #141413; }
        h1 { color: #141413; }
...
""")
    ...
    for h in history:
        ...
        for qinfo in train_queries:
            ...
            html_parts.append(f'            <td class="result {css_class}">{icon}<span class="rate">{r.get("triggers", 0)}/{r.get("runs", 0)}</span></td>\n')
        ...
        for qinfo in test_queries:
            ...
            icon = "\u2713" if did_pass else "\u2717"
            css_class = "pass" if did_pass else "fail"
            html_parts.append(f'            <td class="result test-result {css_class}">{icon}<span class="rate">{r.get("triggers", 0)}/{r.get("runs", 0)}</span></td>\n')
...
def main():
    ...
    parser.add_argument("input", help="JSON output from run_loop.py (or - for stdin)")
