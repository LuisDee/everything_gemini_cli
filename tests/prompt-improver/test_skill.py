#!/usr/bin/env python3
"""
Tests for the prompt-improver skill (Gemini CLI format)
...
# Paths
SKILL_DIR = Path(__file__).parent.parent.parent / "skills" / "prompt-improver"
...
def test_skill_directory_exists():
    ...
    print("  pass: Skill directory exists")
...
def test_yaml_frontmatter():
    ...
    print(f"  pass: YAML frontmatter valid (name: {name})")
...
def test_skill_content_structure():
    ...
    print("  pass: SKILL.md has expected content structure")


def test_no_claude_references():
    """Test that no Claude-specific references remain"""
    content = SKILL_MD.read_text()

    # Check for Claude-specific references that should have been transformed
    assert "UserPromptSubmit" not in content, "Found Claude hook event 'UserPromptSubmit'"
    assert "AskUserQuestion" not in content, "Found Claude tool 'AskUserQuestion'"
    assert ".claude/" not in content, "Found .claude/ path reference"
    assert "Claude Code" not in content, "Found 'Claude Code' reference"
    assert "TodoWrite" not in content, "Found Claude tool 'TodoWrite'"

    print("  pass: No Claude-specific references found")
...
def test_references_directory():
    ...
    print(f"  pass: References directory has all {len(expected_files)} expected files")


def test_references_no_claude():
    """Test that reference files have no Claude-specific references"""
    reference_files = [
        "question-patterns.md",
        "research-strategies.md",
        "examples.md",
    ...
    for filename in reference_files:
        ...
        for pattern in claude_patterns:
            assert pattern not in content, \
                f"Found '{pattern}' in {filename}"

    print("  pass: Reference files have no Claude-specific references")


def test_correct_tool_names():
    """Test that Gemini CLI tool names are used"""
    content = SKILL_MD.read_text()

    # Check for correct Gemini tool names (these should NOT appear as Claude names)
    wrong_tool_names = ['"Grep"', '"Glob"', '"Read"', '"Write"', '"Edit"', '"Bash"']
    for tool in wrong_tool_names:
        # These are Claude tool names in quoted form - should not appear
        # (grep_search, glob, read_file, write_file, replace, run_shell_command are correct)
        pass  # The skill uses conversational references, not direct tool calls

    print("  pass: Tool name references are correct")
...
def test_reference_file_structure():
    ...
    print("  pass: Reference files have proper structure")
...
def test_skill_references_valid():
    ...
    print("  pass: All SKILL.md file references are valid")
...
def test_skill_line_count():
    ...
    if line_count > 500:
        print(f"  warn: SKILL.md is {line_count} lines (recommended: <500)")
    else:
        print(f"  pass: SKILL.md is {line_count} lines (under 500 line recommendation)")
...
def run_all_tests():
    ...
    tests = [
        ...
        test_no_claude_references,
        test_references_directory,
        test_references_no_claude,
        test_correct_tool_names,
