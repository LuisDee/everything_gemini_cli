...
```bash
gemini --prompt /mail                    # Email-only triage
gemini --prompt /slack                   # Slack-only triage
gemini --prompt /today                   # All channels + calendar + todo
gemini --prompt /schedule-reply "Reply to Sarah about the board meeting"
```
