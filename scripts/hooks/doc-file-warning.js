#!/usr/bin/env node
/**
 * Doc file warning hook (BeforeTool - write_file)
...
function isAllowedDocPath(filePath) {
  ...
  if (/^(README|GEMINI|AGENTS|CONTRIBUTING|CHANGELOG|LICENSE|SKILL|MEMORY|WORKLOG)\.md$/i.test(basename)) {
    return true;
  }

  if (/\.gemini\/(commands|plans|projects)\//.test(normalized)) {
    return true;
  }
